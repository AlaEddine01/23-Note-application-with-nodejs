...
const removeFromFile = (path, titleToRemove) => {
  fs.readFile(path, (err, data) => {
    if (err) console.log("Something went wrong!");
      ...
      let jsonContent = JSON.parse(data);
      const textContent = jsonContent.filter(
        (el) => el.title !== titleToRemove
      );
...
switch (process.argv[2]) {
  case "add":
    return addToFile("data.json", process.argv[3], process.argv[4]);
  case "remove":
    return removeFromFile("data.json", process.argv[3]);
  case "read":
    return displayContent("data.json", process.argv[3]);
  default:
